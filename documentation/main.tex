...
at the same port. This is the IP of the  router and it will redirect the connection to the target RPi only if you use this port. Otherwise, you have to change some network configurations.\seealso{See \Cref{sub:network_conf}}.
...
\begin{enumerate}
	...
	\item
	\marginlabel{HIGH PRIORITY} Implement ACPI to the RPi to prevent damage
	\item
	\marginlabel{HIGH PRIORITY} Add a manual override to the valve control (lock/unlock)
	\item
	\marginlabel{HIGH PRIORITY} Configure RPi to connect to the local network of the robot through its Ethernet port
\end{enumerate}
...
\marginlabel{! To be implemented}If you're trying to connect thought a wired connection, the {\tt RPI\_IP} has to be set up manually to an IP in the robot controller's subnet.
